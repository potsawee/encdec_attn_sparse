...
from transformers.modeling_bart_efficient_decoder import BartForConditionalGeneration, SelfAttention
...
class HierSelfAttention(SelfAttention):
    ...
    def forward(
        ...
        # ----- 14 April 2021 ----- #
        assert q.size(1) == 1, "I think decoding token size must be 1 (at inference)!!"
        ...
        sent_k = sent_k.reshape(self.num_heads*bsz, -1, self.head_dim)
        ...
        # topk
        assert rnn_sent_attn_weights.size(1) == 1,  "I think decoding token size must be 1 (at inference)!!"
        sent_topk = torch.topk(rnn_sent_attn_weights.mean(dim=[0,1]), k=min(num_sentences_kept, boundary_matrix.size(-1)), dim=-1)[-1]
        kept_wordids = []
        for sentid in sent_topk:
            kept_wordids += sentid2wordid[sentid]
        # filter k, v
        k = k[:, kept_wordids, :]
        v = v[:, kept_wordids, :]
        src_len = len(kept_wordids)
        if key_padding_mask is not None:
            # assert bsz == 1, "only support batch size = 1 currently"
            key_padding_mask = key_padding_mask[:, kept_wordids]
...
class BartEfficientDecoder(BartForConditionalGeneration):
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            boundary = model_specific_kwargs['boundary']
            _encoder_outputs  = encoder_outputs[0]
            num_sentences = len(boundary['sentid2wordid'])
            batch_size    = _encoder_outputs.size(0)
            # assert batch_size == 1, "batch_size == 1 only!!" ---> batch_size = num_beam... they all are the same
        ...
        if self.config.is_encoder_decoder:
            ...
            sent_outputs    = sent_outputs.index_select(0, expanded_batch_idxs)
            model_specific_kwargs['boundary']['sent_outputs'] = sent_outputs
        else:
            encoder_outputs = None
