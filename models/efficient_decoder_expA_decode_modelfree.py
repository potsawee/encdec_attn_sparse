...
class HierSelfAttention(SelfAttention):
    ...
    def forward(
        ...
        # Hierarchical Sentence-level Attnetion ~ approximation v1 #
        # q => [nheads, tgt_len, 64]
        # k => [nheads, src_len, 64]
        # ---- 22 March 2021 ---- #
        # do phi mapping
        ...
        phi_q = F.elu(q) + 1
        phi_k = F.elu(k) + 1
        phi_sent_k = torch.bmm(phi_k.transpose(1,2),  boundary_matrix)
        phi_prod   = torch.bmm(phi_q, phi_sent_k)

        # keep just the indices, mean over head & decoder time step
        num_sentences_kept = boundary['num_sentences_kept']
        phi_prod_topk = torch.topk(phi_prod.mean(dim=[0,1]), k=min(num_sentences_kept, boundary_matrix.size(-1)), dim=-1)[-1]
        kept_wordids = []
        ...
        # exact_sent_attn_weights = torch.bmm(attn_weights, boundary_matrix)
